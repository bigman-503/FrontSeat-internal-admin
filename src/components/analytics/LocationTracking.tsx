...
import { MapPin, Calendar, Clock, Navigation, Route } from 'lucide-react';
...
export function LocationTracking({ device, timeRange, selectedDate, onDateSelect }: LocationTrackingProps) {
  console.log('🎯 LocationTracking component rendered:', {
  ...
  useEffect(() => {
    console.log('🔍 LocationTracking useEffect triggered:', {
    ...
    loader.load().then(() => {
      ...
      if (mapRef.current) {
        ...
        mapInstance.current = new google.maps.Map(mapRef.current, {
          center,
          zoom: 13,
          mapId: 'DEMO_MAP_ID', // Using same map ID as GoogleMap.tsx
        });
  ...
  const addLocationMarkers = (map: google.maps.Map, locations: LocationData[]) => {
        ...
        if (infoWindowRef.current && mapInstance.current) {
          infoWindowRef.current.setContent(`
            <div style="padding: 8px;">
              ...
              <p style="margin: 0; font-size: 12px; color: #666;">Time: ${formatDateTimePST(location.timestamp)}</p>
  ...
  const addRoutePolyline = (map: google.maps.Map, locations: LocationData[]) => {
  ...
  // Render different views based on timeRange and selectedDate
  console.log('🎨 Determining render view:', {
    timeRange,
    selectedDate,
    shouldShowWeekView: timeRange === '7d' && !selectedDate,
    shouldShowMonthView: timeRange === '30d' && !selectedDate,
    shouldShowMapView: !(timeRange === '7d' && !selectedDate) && !(timeRange === '30d' && !selectedDate)
  });

  if (timeRange === '7d' && !selectedDate) {
    console.log('📅 Rendering WeekLocationChart');
    return (
      <WeekLocationChart
        device={device}
  ...
  if (timeRange === '30d' && !selectedDate) {
    console.log('📅 Rendering MonthLocationChart');
    return (
      <MonthLocationChart
        device={device}
  ...
  // Show detailed map view for 24h or when a specific date is selected
          ...
          <CardTitle className="flex items-center gap-2">
            ...
            {selectedDate && (
              <span className="text-sm text-gray-600 ml-2">
                ({formatDatePST(new Date(selectedDate + 'T00:00:00-07:00'), { 
                  weekday: 'long', 
                  month: 'long', 
                  day: 'numeric', 
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-600">
                        {formatDateTimePST(location.timestamp)}
                      </p>
                    </div>
                  </div>
